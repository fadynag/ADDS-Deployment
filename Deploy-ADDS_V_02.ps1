...
sleep 1
Write-Host "Here is the Modules the script will handle:`
...
switch ($Dis_IPV6)
{
    ...
    'N' {Write-Host "Disabled IPV6 has been discarded" -ForegroundColor Magenta}
...
$Pref_DNS = Read-Host "Enter Prefered DNS Server IP Address"
...
$Dis_FW = Read-Host "Would you like to disable Windows Firewall on this Server? Y / N" 
    Switch($Dis_FW) {
    'Y'
    {
    ...
    'N'
    {
    Write-Host "Please check your Firewall settings later" -ForegroundColor Magenta
    }
}
}
...
Write-Host "Here is the Function Level values for ForestMode and Domain Mode
...
$Forest_Test = Test-ADDSForestInstallation -DomainName $Domain_FQDN -DomainNetbiosName $Domain_NetBios -ForestMode $Forest_FL -DomainMode $Domain_FL -SafeModeAdministratorPassword $Safe_Mode_PSW -NoDnsOnNetwork -WarningAction Ignore -ErrorVariable Failed_Forest -WarningVariable Warning_Forest -Force
